...
app.use('/api/', async (req, res, next) => {
  if (req.user) {
    ...
    if (user.rows[0]?.suspended) return res.status(403).json({ error: 'Account suspended' });
  }
  next();
});
...
app.post('/api/signup', async (req, res) => {
  ...
  try {
    if (!await verifyCaptcha(captcha_token)) return res.status(403).json({ error: 'Captcha failed' });
    ...
    const newUser = await pool.query(
      'INSERT INTO users (email, password, credits, referred_by, referral_code, req_ip_address) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id',
      [email, hashedPass, credits, referred_by, referral_code, req.ip_address]
    );
    await pool.query('INSERT INTO audit_logs (user_id, action, ip_address) VALUES ($1,$2,$3)', [newUser.rows[0].id, 'signup', req.ip_address]);
...
app.post('/api/login', async (req, res) => {
  ...
  const token = jwt.sign({ id: user.rows[0].id, email: user.rows[0].email, role: user.rows[0].role }, process.env.JWT_SECRET);
  await pool.query('INSERT INTO audit_logs (user_id, action, ip_address) VALUES ($1,$2,$3)', [user.rows[0].id, 'login', req.ip_address]);
...
app.get('/api/my-profile', verifyToken, async (req, res) => {
  const user = await pool.query('SELECT email, credits, referral_code FROM users WHERE id=$1', [req.user.id]);
